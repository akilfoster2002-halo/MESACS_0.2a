...
const ringmod = require('./ring');
const RING_HZ = require('../public/bout.js').RULES.hz;
...
/* And the ring. Same two ways out to people, same ignorance of sockets;
   the only difference is which referee is behind it. */
const ring = new ringmod.Ring({
...
wss.on('connection', async (ws, req)=>{
  ...
  ws.on('message', async raw=>{
    ...
    if(m.t==='leave'){
      const was = p.server;
      lobby.cancel(p.id); ring.leave(p.id);
      if(was) broadcastRoom(was,{ t:'left', id:p.id, display:p.display }, ws);
    ...
    /* THE RING. The same shape as the arena above it: what goes up is
       which way I am leaning, and everything about who hit whom is
       decided here and sent back. */
    if(m.t==='arena'){
      if(m.op==='input'){ ring.input(p.id, m.i||{}); return; }
      if(m.op==='cancel'){
        if(ring.cancel(p.id)) ws.send(JSON.stringify({ t:'arena', op:'cancelled' }));
        return;
      }
      if(m.op==='leave'){ ring.leave(p.id); return; }
      if(m.op!=='queue') return;
      if(!p.server){ ws.send(JSON.stringify({ t:'arena', op:'error',
        message:'You are not in a room.' })); return; }
      if(rateLimited('arena:'+p.id, 20, 60000)){
        ws.send(JSON.stringify({ t:'arena', op:'error',
          message:'Too many tries — wait a moment.' })); return; }
      const r=ring.join(p.server, { id:p.id, name:p.display,
                                    robot:m.robot, programs:m.programs });
      if(r.status==='waiting'){
        ws.send(JSON.stringify({ t:'arena', op:'waiting' }));
        broadcastRoom(p.server, { t:'arena', op:'open', name:p.display }, ws);
      } else if(r.status==='rejected'){
        ws.send(JSON.stringify({ t:'arena', op:'rejected', errors:r.errors }));
      } else if(r.status==='error'){
        ws.send(JSON.stringify({ t:'arena', op:'error', message:r.message }));
      }
      return;
    }
...
/* The ring's own heartbeat. Twenty times a second is the tick the
