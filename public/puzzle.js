...
window.PUZZLE = (function(){
  ...
  const OPS=[
    { id:'cam', name:'Blind the Camera', learn:'Loops and counting',
      brief:'A camera sweeps the hall between you and the vault. Get to the <b>terminal</b> in the side office, then reprogram the camera to stare at the wall.',
      device:'camera',
      ...
      guards:[ {route:[[1,6],[8,6],[8,5],[1,5]], speed:2.2, char:'p'} ] }
  ];
  ...
  async function start(n){
    ...
    G.pos.set(sp.x*U, 1.7, sp.z*U); G.yaw=0; G.pitch=0;
    if(window.AVATAR) AVATAR.attach();

    // guards, as real characters
    for(const g of (O.guards||[])){
      const mesh = await loadChar(g.char||'p');
      mesh.position.set(g.route[0][0]*U, 0, g.route[0][1]*U);
    ...
    if(cs){
      ...
      grp.position.set(cs.x*U,0,cs.z*U);
      G.roomGroup.add(grp);
    ...
    if(ts){
      ...
      m.position.set(ts.x*U,1.1,ts.z*U); G.roomGroup.add(m);
      L.terminal={ x:ts.x, z:ts.z, mesh:m };
    }
    ...
    if(vs){
      ...
      m.position.set(vs.x*U,1.2,vs.z*U); G.roomGroup.add(m);
      L.vault={ x:vs.x, z:vs.z, mesh:m };
    }
  ...
  async function loadChar(id){
    if(window.AVATAR) { try{ return await AVATAR.load(id); }catch(e){} }
  ...
  function update(dt){
    ...
    if(p){
      ...
      if(L.near==='vault')    p.innerHTML= L.blinded ? t('<kbd>E</kbd> open the vault')
                                                     : t('The camera is still watching the vault.');
    }
  }
  function dist(o){ return Math.hypot(o.x*U-G.pos.x, o.z*U-G.pos.z); }
  ...
  function use(){
    ...
    if(L.near==='vault'){
      if(!L.blinded){ brief(t('Not while the camera is sweeping.')); return; }
      win();
    }
  }
  ...
  function hud(){
    ...
    document.querySelector('#objList').innerHTML=
      `<li class="${L.blinded?'done':'cur'}">📷 ${t('Blind the camera from the terminal')}</li>
       <li class="${L.done?'done':''}">💾 ${t('Reach the vault')}</li>
