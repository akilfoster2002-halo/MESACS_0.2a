...
/* THE FACES, however they are stored. This assumed for a long time that
   every three vertices in a row were a triangle, which is true of the
   unindexed exports it was written for and silently false of anything
   indexed — it read a remap table as a face list and reported a fifth of
   the triangles with no complaint. */
let FACE;
{
  const pi=prim.indices;
  if(pi===undefined){ FACE=new Uint32Array(acc.count); for(let i=0;i<acc.count;i++) FACE[i]=i; }
  else {
    const ia=g.accessors[pi], ibv=g.bufferViews[ia.bufferView];
    const at=(ibv.byteOffset||0)+(ia.byteOffset||0);
    FACE=new Uint32Array(ia.count);
    for(let i=0;i<ia.count;i++)
      FACE[i] = ia.componentType===5125 ? bin.readUInt32LE(at+i*4)
              : ia.componentType===5123 ? bin.readUInt16LE(at+i*2)
              : bin.readUInt8(at+i);
  }
}
...
for(let f=0;f<FACE.length;f+=3){
  const a=idx[FACE[f]], c=idx[FACE[f+1]], d=idx[FACE[f+2]];
...
/* Report what actually happened. These numbers were once written as
   literals from the first model this was run on, which made every later
   run quietly describe that one instead of itself. */
const inTris=FACE.length/3;
console.log('tolerance      ', EPS);
console.log('vertices  ', acc.count.toLocaleString(), '->', nv.toLocaleString(),
            '('+(100-100*nv/acc.count).toFixed(1)+'% merged)');
console.log('triangles ', inTris.toLocaleString(), '->', tri.toLocaleString(),
            degenerate?('  ('+degenerate.toLocaleString()+' collapsed, '
              +(100*degenerate/inTris).toFixed(1)+'%)'):'');
