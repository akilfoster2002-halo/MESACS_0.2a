...
/* the clip already in the base file */
if(g.animations[0]) g.animations[0].name=extra.length?process.env.BASE_NAME||'idle':'idle';

const names=g.nodes.map(n=>n.name);
for(const {name,file} of extra){
  const S=read(file);
  const sn=S.json.nodes.map(n=>n.name);
  if(sn.length!==names.length || !sn.every((n,i)=>n===names[i]))
    throw new Error('skeleton mismatch in '+file+' — cannot merge by node index');
  const anim=S.json.animations[0];
  const samplers=anim.samplers.map(s=>({
    input:addAccessor(S.json,S.bin,s.input),
    output:addAccessor(S.json,S.bin,s.output),
    interpolation:s.interpolation||'LINEAR' }));
  g.animations.push({ name,
    samplers,
